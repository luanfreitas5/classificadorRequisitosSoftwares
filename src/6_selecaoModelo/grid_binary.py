...
def model_select(dataset):
    ...
    x_train, _, y_train, _ = train_test_split(x, y, test_size=.3, random_state=183212)
    
    cv = KFold(n_splits=10, shuffle=True, random_state=183212)
    
    parametros = {'SVM':{'C':[0.1, 1, 10], 'gamma':[0.01, 0.1, 1]},
                  'RandomForestClassifier':{'n_estimators': np.arange(80, 100, 10), 'max_depth': np.arange(3, 5), 'criterion':['gini', 'entropy']},
                  'MLPClassifier':{'hidden_layer_sizes': [(10, 30, 10), (20,)], 'activation': ['tanh', 'relu'], 'solver': ['sgd', 'adam']},
...
if __name__ == '__main__':
    ...
    print("Fim da selecao dos hiperparemetros para classes binaria")
